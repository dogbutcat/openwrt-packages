...
nowmd5=${nowmd5%% *}
local lastmd5=$(uci get MosChinaDNS.MosChinaDNS.gfwlistmd5 2>/dev/null)
if [ "$nowmd5" != "$lastmd5" ]; then
	uci set MosChinaDNS.MosChinaDNS.gfwlistmd5="$nowmd5"
	uci commit MosChinaDNS
	[ "$1" == "noreload" ] || /etc/init.d/MosChinaDNS reload
fi
}
configpath=$(uci get MosChinaDNS.MosChinaDNS.configpath 2>/dev/null)
[ "$1" == "del" ] && sed -i '/programaddstart/,/programaddend/d' $configpath && checkmd5 "$2" && exit 0
gfwupstream=$(uci get MosChinaDNS.MosChinaDNS.gfwupstream 2>/dev/null)
