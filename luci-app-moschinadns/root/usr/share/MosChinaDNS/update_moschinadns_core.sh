...
binpath=$(uci get MosChinaDNS.MosChinaDNS.binpath)
if [ -z "$binpath" ]; then
uci set MosChinaDNS.MosChinaDNS.binpath="/tmp/MosChinaDNS/MosChinaDNS"
binpath="/tmp/MosChinaDNS/MosChinaDNS"
fi
mkdir -p ${binpath%/*}
upxflag=$(uci get MosChinaDNS.MosChinaDNS.upxflag 2>/dev/null)

check_if_already_running(){
	running_tasks="$(ps |grep "MosChinaDNS" |grep "update_moschinadns_core" |grep -v "grep" |awk '{print $1}' |wc -l)"
		...
		doupdate_moschinadns_core
			...
			if [ ! -z "$upxflag" ]; then
				...
				if [ $filesize -gt 8000000 ]; then
					...
					doupx
					mkdir -p "/tmp/MosChinaDNSupdate/MosChinaDNS" >/dev/null 2>&1
					rm -fr /tmp/MosChinaDNSupdate/MosChinaDNS/${binpath##*/}
					/tmp/upx-${upx_latest_ver}-${Arch}_linux/upx $upxflag $binpath -o /tmp/MosChinaDNSupdate/MosChinaDNS/${binpath##*/}
					rm -rf /tmp/upx-${upx_latest_ver}-${Arch}_linux
					/etc/init.d/MosChinaDNS stop nobackup
					rm $binpath
					mv -f /tmp/MosChinaDNSupdate/MosChinaDNS/${binpath##*/} $binpath
					/etc/init.d/MosChinaDNS start
					echo -e "finished"
				fi
			fi
			EXIT 0
...
doupdate_moschinadns_core(){
	...
	*)
	...
	grep -v "^#" /usr/share/MosChinaDNS/links.txt >/tmp/run/MCDlinks.txt
	while read link
	do
		eval link="$link"
		echo -e "try download: $link"
		$downloader /tmp/MosChinaDNSupdate/${link##*/} "$link" 2>&1
		if [ "$?" != "0" ]; then
			echo "download failed try another download"
			rm -f /tmp/MosChinaDNSupdate/${link##*/}
		else
			local success="1"
			break
		fi 
	...
	if [ "${link##*.}" == "zip" ]; then
		...
		downloadbin="/tmp/MosChinaDNSupdate/${link##*/}"
	fi
	...
	if [ "$?" == "1" ]; then
		echo "mv failed maybe not enough space please use upx or change bin to /tmp/MosChinaDNS" 
		EXIT 1
	fi
	/etc/init.d/MosChinaDNS start
	rm -rf "/tmp/MosChinaDNSupdate" >/dev/null 2>&1
...
EXIT(){
	rm /var/run/update_moschinadns_core 2>/dev/null
	[ "$1" != "0" ] && touch /var/run/update_moschinadns_core_error
	exit $1
}
main(){
	...
	touch /var/run/update_moschinadns_core
	rm /var/run/update_moschinadns_core_error 2>/dev/null
	main $1
