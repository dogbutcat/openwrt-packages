...
check_latest_version(){
	check_wgetcurl
	latest_ver="$($downloader - https://api.github.com/repos/IrineSistiana/mosdns/releases/latest 2>/dev/null|grep -E 'tag_name' |grep -E 'v[0-9.]+' -o 2>/dev/null)"
...
doupdate_mosdns_core(){
	...
	*)
	...
	if [ "${link##*.}" == "zip" ]; then
		...
		if [ ! -e "/tmp/MosDNSupdate/MosDNS/mosdns" ]; then
			echo -e "Failed to download core."
			rm -rf "/tmp/MosDNSupdate" >/dev/null 2>&1
			EXIT 1
		else
			mv "/tmp/MosDNSupdate/MosDNS/mosdns" "/tmp/MosDNSupdate/MosDNS/MosDNS"
		fi
	...
	/etc/init.d/MosDNS stop nobackup
	rm "$binpath"
	if [ -e "/tmp/MosDNSupdate/chn.list" ]; then
		mv -f "/tmp/MosDNSupdate/chn.list" "/usr/share/MosDNS"
	fi
